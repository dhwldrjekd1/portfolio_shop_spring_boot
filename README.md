...
- **회원별 리뷰 조회 API에 인가 검증 누락** (2026-07-20) — `GET /api/review/member/{loginId}`에 `SessionAuth` 체크가 빠져있어, 로그인 여부와 무관하게 loginId만 알면 누구나 다른 회원의 리뷰 작성 내역을 조회할 수 있던 문제. 다른 마이페이지 조회 API(`/api/order/{loginId}` 등)와 동일하게 `isSelfOrAdmin` 검증 추가.
- **리뷰 등록 시 구매확인/중복방지가 클라이언트에서만 검증됨** (2026-07-20) — 배송완료 구매 여부·중복 작성 여부를 확인하는 `/api/review/check/*` 엔드포인트는 프론트가 폼을 보여주기 전에만 호출했고, 실제 저장 엔드포인트(`POST /api/review`)는 이 확인을 거치지 않아 API를 직접 호출하면 구매하지 않은 상품에도, 이미 작성한 상품에도 무제한으로 리뷰를 등록할 수 있던 문제. `BaseReviewService.save()`에 동일한 검증을 서버측에서 재확인하도록 추가하고, 동시 요청으로 인한 레이스까지 막기 위해 `reviews(item_id, login_id)`에 DB 유니크 제약도 추가.
...
- **시드 카탈로그(products.json) 상품이 관리자 페이지에서 안 보이고 수정도 안 됨** (2026-07-21) — 관리자 "재고 관리"/"상품 관리" 탭은 `/api/item`(DB `items` 테이블) 응답만 표시하는데, 화면에 보이는 상품 대부분은 정적 시드 카탈로그(`products.json`)에서 온 것이라 DB에 대응 행이 없어 관리자 화면에 아예 노출되지 않고 수정도 불가능했음(코드 버그는 아니고, 애초에 "시드+DB 오버라이드" 이원 구조였음). `products.json`의 22개 상품을 DB `items` 테이블에 동일 id로 일괄 삽입해 해결 — **코드 변경 없이 데이터만 마이그레이션**(작업 전 전체 DB 백업, 트랜잭션 처리, `id`가 `GENERATED ALWAYS AS IDENTITY`라 `OVERRIDING SYSTEM VALUE`로 명시 삽입 후 시퀀스 재설정, 원본과 전 필드 전수 비교로 검증). `images`/`colors`/`sizes`/`tags`/`colorProducts`처럼 DB 스키마에 없는 필드는 여전히 `products.json`에서 그대로 표시되므로 이번 작업으로 영향 없음. 이후 관리자가 등록하는 완전히 새로운 상품은 그대로 auto-increment id(23부터)로 발급됨.
- **관리자 강퇴/권한변경이 로그인된 세션에 즉시 반영 안 됨** (2026-07-28) — `SessionAuth`가 로그인 시점에 세션에 저장해둔 role만 확인하고 DB를 재조회하지 않아서, 관리자가 회원을 강퇴하거나 권한을 바꿔도 이미 로그인된 세션은 만료 전까지 예전 권한을 그대로 쓰던 문제. 매 `/api/**` 요청마다 세션의 role을 DB와 동기화하고 강퇴(banned)된 계정은 세션을 즉시 무효화하는 `SessionRoleSyncInterceptor` 추가. 이 과정에서 `MemberController.updateRole`이 role 값을 검증 없이 그대로 저장해 role이 null/이상값이면 인터셉터가 예외를 던질 수 있는 것도 함께 발견해, role을 `user`/`admin`/`banned` 화이트리스트로 검증하도록 추가함.
- **로그인 실패 잠금이 계정(loginId) 단위로만 걸림** (2026-07-28) — 브루트포스 방어가 loginId만으로 카운트돼서, 공격자가 임의의 계정 loginId로 오답을 반복하면 계정 주인이 정상 비밀번호로 로그인해도 함께 잠기던 문제. 잠금 키를 loginId+요청 IP 조합으로 바꿔, 공격자 IP만 잠기고 계정 주인의 다른 IP 접속은 막히지 않도록 수정.
- **상품 가격/할인율에 서버측 범위 검증 없음** (2026-07-28) — 할인율 0~100, 가격 0 이상 제한이 프론트 입력창(`min`/`max`)에만 있어서, API를 직접 호출하면 할인율 100 초과나 음수 가격으로 저장할 수 있었고 이 경우 결제 금액 계산이 어긋날 수 있던 문제. `Item` 엔티티(생성자/`update`/`updateDiscountRate`)에 서버측 검증 추가.
- **구매 여부 확인 API의 N+1 쿼리** (2026-07-28) — `GET /api/review/check/purchased/{loginId}/{itemId}`가 회원의 전체 주문과 각 주문의 주문상품(지연 로딩)을 다 불러와 스트림으로 순회해서, 주문이 많은 회원일수록 상품 상세페이지 로딩이 느려지던 문제. `existsByLoginIdAndStatusAndOrderItems_ItemId` 파생 쿼리로 교체해 단일 쿼리로 처리.
- **(교차검증에서 발견) 로그인 잠금을 IP 단위로 바꾸면서 계정 전체 상한이 사라짐** (2026-08-02) — 바로 위 "로그인 실패 잠금 IP화" 수정은 공격자 한 명이 한 IP로 계정 주인을 잠가버리는 것은 막았지만, 그 대가로 계정 단위 전역 상한이 없어져서 여러 IP(봇넷 등)로 나눠 공격하면 IP당 5회씩 사실상 무제한으로 같은 계정에 비밀번호를 시도할 수 있게 된 부작용이 있었음. `LoginAttemptService`에 (loginId+IP) 잠금(5회)과 별개로 loginId 단독 전역 잠금(20회)을 함께 두어, 분산 공격도 계정 단위로는 결국 잠기도록 보강. 실제로 서로 다른 IP 4개에서 5회씩(총 20회) 오답을 보낸 뒤, 한 번도 안 쓴 새 IP에서 정답으로 로그인해도 전역 잠금으로 차단되는 것을 확인했고, 기존 목적(공격자 IP만 잠기고 계정 주인의 다른 IP 접속은 안 막힘)도 그대로 유지되는 것을 재확인함.
