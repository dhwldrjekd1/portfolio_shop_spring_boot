...
| DB | PostgreSQL |
| 빌드 도구 | Gradle |
| 보안 | Spring Security (세션 기반 인증, CSRF 토큰) |
| 결제 | 토스페이먼츠 API |
...
- 회원가입 / 로그인 / 로그아웃 (세션 기반)
- 회원 정보 조회·수정·탈퇴
- 비밀번호 찾기 (임시 비밀번호 발급, 이메일로만 전달)
- 비밀번호 정책 서버측 강제 (8자 이상 + 영문 소문자·숫자·특수문자, `PasswordPolicy`)
- 로그인 brute-force 방어 (계정당 5회 연속 실패 시 5분 잠금, `LoginAttemptService`)
- 구매금액 기반 자동 등급 업데이트 (브론즈 / 실버 / 골드 / 플래티넘)
- 세션 기반 서버측 권한 검증 (`SessionAuth`) — 관리자 전용 API 및 본인 소유 데이터 API를 컨트롤러 단에서 직접 검증
- CSRF 방어 (쿠키 기반 토큰) — 상태변경 요청은 유효한 토큰 없이는 거부됨
...
- 장바구니 담기 / 수량 변경 / 삭제
- 토스페이먼츠 결제 승인 API 연동
- 주문 생성 시 서버가 실제 상품가(할인 반영)로 금액을 재계산해 대조 — 클라이언트가 보낸 금액이 다르면 주문 거부
...
### 공통
- 예외 응답 정제 (`ApiError`) — DB/JPA 계층 예외는 서버 로그에만 상세를 남기고 클라이언트에는 일반화된 메시지만 반환
